...
module DEA.ParsingToSmartContract (toSmartContract, toSmartContractWithError) where
...
toSmartContract contractSpecification = let scName = upperCaseFirstChar (contractName contractSpecification)
                                          in unlines $
                                                ...
                                                , "\tfunction " ++ scName ++ "(){"
                                                ,  addTabsToString (display (initialisation contractSpecification))
                                                , "\t}"
                                                , addTabsToString $ unlines $ (map (toMonitoringLogic) (daes contractSpecification)) 
                                                ,"}"]

addTabsToString :: String -> String
addTabsToString text = unlines (map ("\t" ++) (lines text))
...
transitionToCheck dea (Transition _src _dst (GCL event Nothing (Just action))) = unlines $ 
                                                                              ["if(_" ++ (daeName dea) ++ "State == " ++ "\"" ++ display _src ++ "\""
                                                                                 ++ (if (eventCondition event == "") then "" else ("\n&& " ++ eventCondition event))
                                                                                ,"){"
                                                                                , addTabsToString $ display action
                                                                                , "\t_" ++ (daeName dea) ++ "State = " ++ "\"" ++ display _dst ++ "\";"
                                                                                , "}"]
...
createEventCheck dea e = let trans = getTransitionsOfAnEvent (transitions dea) e
                             in unlines $
                                ["function " ++ eventMethodLabel e ++ "(" ++ variableListAssociatedWithEvent e ++ "){"
                                  , addTabsToString $ unlines $ (map (transitionToCheck dea) (trans))
                                  , "}"]



toMonitoringLogic :: DEA -> String
toMonitoringLogic dea = addTabsToString $ unlines $ 
